...
import com.example.presentation.common.extension.toDisplayName
...
fun ChurchInfoScreen(
        ...
        Column(
            ...
            Column(
                ...
                TebahTextField(
                    ...
                    label = stringResource(R.string.church_name_label),
                    isError = state.churchNameError != null,
                    errorMessage = state.churchNameError?.let { stringResource(it) }
                )

                TebahTextField(
                    value = state.region?.toDisplayName() ?: "",
                    onValueChange = {},
                    label = stringResource(id = R.string.church_region_label),
                    readOnly = true,
                    onClick = { viewModel.showRegionSelector() },
                    isError = state.regionError != null,
                    errorMessage = state.regionError?.let { stringResource(it) }
                )

                TebahTextField(
                    value = state.phoneNumber,
                    onValueChange = viewModel::onPhoneNumberChange,
                    label = stringResource(R.string.church_phone_label),
                    isError = state.phoneNumberError != null,
                    errorMessage = state.phoneNumberError?.let { stringResource(it) }
                )

                TebahTextField(
                    value = state.churchIntro,
                    onValueChange = viewModel::onChurchIntroChange,
                    label = stringResource(R.string.church_intro_label),
                    isError = state.churchIntroError != null,
                    errorMessage = state.churchIntroError?.let { stringResource(it) }
                )
            }
            ...
            Column(
                ...
                LargeButton(
                    ...
                    enabled = state.isChurchInfoValid
                )
            }
        }
    }
    ...
    // 지역 선택 모달
    if (state.regionSelectorVisible) {
        RegionSelectorModal(
            selectedRegion = state.region,
            onDismissRequest = { viewModel.dismissRegionSelector() },
            onRegionSelected = {
                viewModel.onRegionChange(it)
                viewModel.dismissRegionSelector()
            }
        )
    }
}
