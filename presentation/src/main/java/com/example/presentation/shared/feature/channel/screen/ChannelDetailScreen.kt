...
import com.example.presentation.shared.feature.user.screen.PostPreviewCard
...
fun PreviewChannelDetailScreen() {
    ...
    val dummyPainter = painterResource(id = android.R.drawable.sym_def_app_icon)
    ...
    val fakePosts = listOf(
        PostData(
            ...
            userId = "user123",
            postTime = "10분 전",
            previewText = "이번 주 금요모임은 7시에 시작합니다.",
            likeCount = 12,
            saveCount = 3,
            checkCount = 5,
            isNotice = true
        ...
        PostData(
            ...
            userId = "user456",
            postTime = "1시간 전",
            previewText = "청년부 캠핑 신청 마감되었습니다.",
            imageList = listOf(dummyPainter),
            likeCount = 7,
            saveCount = 4,
            checkCount = 1,
            isNotice = false
    ...
    val previewState = ChannelDetailState(
        channel = fakeChannel,
        posts = fakePosts,
        isChannelOwner = true,
    ...
    ChannelDetailScreenPreviewHost(state = previewState)
}

@Composable
fun ChannelDetailScreenPreviewHost(state: ChannelDetailState) {
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(horizontal = Paddings.layout_horizontal, vertical = Paddings.layout_vertical)
